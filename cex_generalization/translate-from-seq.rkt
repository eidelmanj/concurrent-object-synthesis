      ...
      (list
       ...
       (Set-var (get-return-var (Run-method-args (first instr-list)))
                (New-struct (substring (Run-method-method (first instr-list))
                            ...
                            (list (None) (None) (None) (None)) ;; TODO: Note this only works for Node
                            ;; (map
                            ;;  get-var-id
...
(define (find-create-vars l arg-list defined-list)
  (cond
    [(empty? l) `()]
    [(Single-branch? (first l))
     (append
      (find-create-vars (Single-branch-branch (first l) ) arg-list defined-list)
      (find-create-vars (rest l) arg-list defined-list))]
     [(Branch? (first l))
      (append
       (find-create-vars (Branch-branch1 (first l)) arg-list defined-list)
       (find-create-vars (Branch-branch2 (first l)) arg-list defined-list))]

     [
      (and
       (Create-var? (first l))
       (Set-var? (Create-var-id (first l))))
      (append
       (list
        (Create-var (Set-var-id (Create-var-id (first l))) (Create-var-type (first l)))
        (repair-expression (Create-var-id (first l)) arg-list defined-list))
       (find-create-vars (rest l) arg-list defined-list) )]
     [(and (Create-var? (first l))
           (not (Set-var? (Create-var-id (first l)))))
      
      (append (list (first l)) (find-create-vars (rest l) arg-list defined-list) )]
     [else
      (find-create-vars (rest l) arg-list defined-list)]))
